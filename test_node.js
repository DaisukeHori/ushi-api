...
async function searchCattle(id) {
    ...
    console.log("Agreement Response Status:", response2.status);
    console.log("Agreement Response HTML snippet:", response2.data.substring(0, 500));
    ...
    console.log("Number of forms:", $("form").length);
    const tokens2 = getTokens($);
    console.log("Tokens for search:", tokens2);
    ...
    if (tables.length >= 2) {
        ...
        for (let i = 1; i < historyRows.length; i++) {
            ...
            if (cols.length >= 6) { // It likely has 6 columns including the row number
                history.push({
...
const testId = process.argv[2] || "1083079037";
searchCattle(testId)
